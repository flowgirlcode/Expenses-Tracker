...
const ExpensesByMonth = () => {
  ...
  const fetchExpenses = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/v1/month-expense');
      setExpensesByMonth(response.data);
    } catch (error) {
      ...
      {selectedMonth && (
        <div className='card'>
          ...
          <p>Total Expense: ₹{calculateTotalExpense(
            expensesByMonth.find(month => month._id === selectedMonth)?.expenses || []
          ).toFixed(2)}</p>
        </div>
      )}
    </MonthStyled>
